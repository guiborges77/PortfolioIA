...
import { MapPin, Calendar, ExternalLink, Briefcase, ChevronRight, Star, Zap, Code } from 'lucide-react';
        ...
        <div className="grid lg:grid-cols-4 gap-8">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center mb-3">
                        ...
                        <div>
                          <h3 className="font-bold text-sm leading-tight">{exp.company}</h3>
                          <p className="text-xs opacity-70 mt-1">{exp.position}</p>
                        </div>
                      </div>
                      <div className="flex items-center">
                        <span className={`px-2 py-1 text-xs rounded-full ${
                          ...
                          {exp.status === 'current' ? t('experience.current') : t('experience.completed')}
                        </span>
                      </div>
                    </div>
                    <ChevronRight 
          ...
          <div className="lg:col-span-3">
            <div className="bg-gray-900/50 backdrop-blur-sm rounded-2xl border border-gray-700/50 overflow-hidden">
              ...
              <div className={`h-1 bg-gradient-to-r ${currentExperience.color}`}></div>
                ...
                <div className="mb-8">
                  <div className="flex items-center mb-6">
                    ...
                    <div className="ml-4">
                      ...
                      <p className={`text-xl font-semibold bg-gradient-to-r ${currentExperience.color} bg-clip-text text-transparent`}>
                  ...
                  <div className="flex flex-wrap gap-4 mb-6">
                    ...
                    <div className="flex items-center">
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                        ...
                        {currentExperience.status === 'current' ? t('experience.current') : t('experience.completed')}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="space-y-3">
                      {currentExperience.activities.map((activity, index) => (
                        <div key={index} className="flex items-start group">
                          <div className={`w-2 h-2 bg-gradient-to-r ${currentExperience.color} rounded-full mt-2 mr-3 flex-shrink-0 group-hover:scale-125 transition-transform`}></div>
                          <span className="text-gray-300 text-sm group-hover:text-white transition-colors">{activity}</span>
                        </div>
                      ))}
                    </div>
                  </div>
